...
# ─── Gift box invite tracking ─────────────────────────────────────────────────

def get_gift_box_invites() -> dict:
    return load("gift_box_invites", {}) or {}

def save_gift_box_invites(data: dict) -> None:
    save("gift_box_invites", data)

def is_gift_box_unlocked(event_id: str, user_id: int) -> bool:
    return get_gift_box_invites().get(f"{event_id}:{user_id}", {}).get("unlocked", False)

def get_gift_box_invite_count(event_id: str, user_id: int) -> int:
    return len(get_gift_box_invites().get(f"{event_id}:{user_id}", {}).get("invitees", []))

def record_gift_box_referral(event_id: str, creator_id: int, invitee_id: int) -> dict:
    """Credit an invite for creator. Returns {added, total, reason?}."""
    if creator_id == invitee_id:
        return {"added": False, "reason": "self"}
    invites = get_gift_box_invites()
    key = f"{event_id}:{creator_id}"
    if key not in invites:
        invites[key] = {"invitees": [], "unlocked": False, "created_at": datetime.now().isoformat()}
    rec = invites[key]
    if invitee_id in rec.get("invitees", []):
        return {"added": False, "reason": "duplicate", "total": len(rec["invitees"])}
    rec["invitees"].append(invitee_id)
    invites[key] = rec
...
def mark_gift_box_unlocked(event_id: str, user_id: int) -> None:
    ...
    key = f"{event_id}:{user_id}"
    if key not in invites:
        invites[key] = {"invitees": [], "unlocked": True, "created_at": datetime.now().isoformat()}
    else:
